...
{
    public partial class SBT2 : Form
    {
        ...
        private void Form28_2_Load(object sender, EventArgs e)
        {
            // TODO: This line of code loads data into the 'DataSet28.tbldata_28_2' table. You can move, or remove it, as needed.
            getbatch();
            ...
            try
            {
               // batchcombo.SelectedIndex = -1;
                ...
                try
                {
                    ...
                    connection.Open();
                    var query = "select distinct (`mbatch`) FROM tbldata_28_2 where `date` like('" + theDate + "%')  ;";
        ...
        void nameplate()
        {
            if (batchcombo.SelectedIndex != -1)
            {
                try
                {
                    connection = new MySqlConnection(connectionString);
                    connection.Open();
                    var query = "select count(*) FROM nameplate28_2 where `date` like('" + theDate + "%') and batchno='" + batchcombo.SelectedItem.ToString() + "'  ;";
        ...
        void genraterpt()
        {
            ...
            try
            {
                ...
                this.tbldata_28_2TableAdapter.Fill(this.DataSet28.tbldata_28_2, batchcombo.SelectedItem.ToString());
        ...
        void retrivenameplate()
        {
            ...
            try
            {
                connection = new MySqlConnection(connectionString);
                connection.Open();
                var query = "select distinct * FROM nameplate28_2 where `date` like('" + theDate + "%') and batchno='" + batchcombo.SelectedItem.ToString() + "' ;";
        ...
        private void btnok_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string query = "INSERT INTO nameplate28_2 (`date`,  `batchno`,`shift`, `supervisor`) VALUES('" + dateTimePicker1.Value.ToString("yyyy-MM-dd") + "','" + batchcombo.SelectedItem.ToString() + "','" + shifttxt.Text + "',' ');";
        ...
        private void batchcombo_DropDownClosed(object sender, EventArgs e)
        {

        }

        private void Form28_2_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyData==key)
            {
        ...
        void gridfill()
        {
            try
            {
                ...
                GlobalClass.adap = new MySqlDataAdapter("SELECT mbatch as `Batch No`, sleeperno as `Sleeper No`, DATE_FORMAT(`date`, '%Y-%m-%d %T') as `Date`,castdate as `Cast Date`, top as `Top`, bottom as `Bottom`, mr1 as `MR1`, mr2 as `MR2`, mf1 as `MF1`, mf2 as `MF2`, mix_design as `Mix Design` FROM tbldata_28_2 where mbatch = '" + batch + "' ;", myConn7);
        ...
        //    Opereation_Form28_2 op = new Opereation_Form28_2(str, i);
        ...
        private void dataGridView1_CellContentDoubleClick_1(object sender, DataGridViewCellEventArgs e)
        {
            str = dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString();
            i = dataGridView1.Rows[e.RowIndex].Index;
            SBT2_Opereation op = new SBT2_Opereation(str, i);
            //Opereation op = new Opereation(str, i);
            op.Show();
        }
    }
